...
#ifndef ZEPETO_CLOTH_CORE_FORWARD_INCLUDED
#define ZEPETO_CLOTH_CORE_FORWARD_INCLUDED
...
#include "ZepetoClothBRDF.cginc"
...
#ifdef _CLOTH
        half3 sheenColor = SampleSheenColor(i.tex.xy);
        half sheenRoughness = SampleSeenRoughness(i.tex.xy);
        half3 c = BRDFCloth(s.diffColor, s.specColor, s.oneMinusReflectivity, s.smoothness, s.normalWorld, -s.eyeVec,
    ...
    half4 fragForwardBaseInternalGrading(VertexOutputForwardBase i)
    {
        ...
        half3 sheenColor = SampleSheenColor(i.tex.xy);
        half sheenRoughness = SampleSeenRoughness(i.tex.xy);
        half4 c = BRDFCloth(s.diffColor, s.specColor, s.oneMinusReflectivity, s.smoothness, s.normalWorld, -s.eyeVec,
                                  gi.light, gi.indirect,
                                  sheenColor, sheenRoughness);
#else
...
#endif // ZEPETO_CLOTH_CORE_FORWARD_INCLUDED
